...
function fallbackParse(command) {
    ...
    if (isAddStock(lower)) {
        ...
        return { type: 'ADD_STOCK', data: { intent: 'ADD_STOCK', product_name: productGuess, quantity } };
    }
    if (isRecordSale(lower)) {
        const quantity = toInt(numberMatches[0], 1);
        const amount = toInt(numberMatches[1], 0);
        return { type: 'RECORD_SALE', data: { intent: 'RECORD_SALE', product: productGuess, quantity, amount } };
    }
    if (isAddCredit(lower)) {
        const amount = toInt(numberMatches[0], 0);
        const party_name = productGuess || 'Party';
        return { type: 'ADD_CREDIT', data: { intent: 'ADD_CREDIT', party_name, amount } };
    }
    if (isCheckBalance(lower)) {
        const party_name = productGuess || 'Party';
        return { type: 'CHECK_BALANCE', data: { intent: 'CHECK_BALANCE', party_name } };
    }
